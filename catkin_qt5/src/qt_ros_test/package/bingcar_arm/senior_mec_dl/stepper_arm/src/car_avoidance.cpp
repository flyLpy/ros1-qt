...
#include <math.h>
#include <iostream>
#include <stepper_arm/position.h>
...
返回  值：无
**************************************************************************/
void current_position_Callback(const stepper_arm::position& msg)	
{
...
int distance_judgment(void)
{
	...
	if(distance1<=0.5) 
	{
		cout<<"obstacle distance less than 0.5m, stop searching."<<endl;
		return 1;
	}
	else
		return 0;	
...
{
	if(cmd_vel_msg.linear.x > 0 && (dis_angleX > 2.356 || dis_angleX < -2.356))
		return 1;

	else if(cmd_vel_msg.linear.x < 0 && dis_angleX > -0.785 && dis_angleX < 0.785)
		return 1;
